public class Solution {
    ...
    public String topological(HashMap<Character, ArrayList<Character>> adj, int[] chars) {
        ...
        while (!st.isEmpty())
            s += st.pop();
        // characters have not occured in any edges. just append them to the
        // result
        for (int i = 0; i < 26; i++) {
            if (chars[i] == 1) {
                s += (char) ('a' + i);
    ...
    public boolean DFS(char c, HashMap<Character, ArrayList<Character>> adj, Stack<Character> st, boolean[] visited,
    ...
    public HashMap<Character, ArrayList<Character>> build(String[] words, int[] chars) {
